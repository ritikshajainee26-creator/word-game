...
// In-Memory Fallback Storage (Used strictly for unit test environments without PG daemon)
let inMemoryMatches = [];
...
async function saveMatchResult(matchData) {
  ...
  return { success: true, matchId, storage: isPgConnected ? 'postgresql' : 'in_memory' };
}
